...
module.exports = function(db){
  ...
  router.get('/', function(req, res, next) {
    if (req.session.user){
      if(req.query.username && req.query.username != req.session.user.username){
        console.log('you can only check your own info.');
        hint = 'Only access to your own data: )';
      }
      res.redirect('/detail');
    } 
    else res.render('signup',{user:{username:'', password:''}});
  });
  ...
  router.get('/detail', function(req, res, next) {
    ...
    console.log(req.session.user);
    res.render('../views/detail.jade', {user:req.session.user, hint:hint});
